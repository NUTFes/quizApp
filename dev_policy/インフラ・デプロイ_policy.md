...
## 🔴 「開発では通るのに本番だけ落ちる」差分の一覧

`localhost:3000` の件と同じ構造の事故が、**フロントの依存関係でも2種類起きる。**
どちらも共通する原因は「**開発環境のほうが本番より緩い**」こと。
緩いほうで動作確認しているので、厳しいほうで初めて落ちる。

### 1. `package.json` への追記漏れ

```
[vite]: Rollup failed to resolve import "qrcode.react"
        from "/app/src/features/monitor/parts/MonitorQrCode.tsx"
```

`import` を書いたが `package.json` に依存を足していない状態。

**開発で気づけない理由:** Vite の開発サーバーは import を**遅延解決**する。
そのモジュールを使う画面を開いた瞬間に初めて解決を試みるので、
`/monitor` を開かなければエラーにならない。
本番の `pnpm build` は全モジュールを事前にバンドルするため、確実に落ちる。

### 2. `pnpm-lock.yaml` のコミット漏れ

```
ERR_PNPM_OUTDATED_LOCKFILE
```

`package.json` は更新したが lock を同じコミットに入れていない状態。

**開発で気づけない理由:** 開発用 `Dockerfile` は `pnpm install`、
本番の `Dockerfile.prod` は **`pnpm install --frozen-lockfile`**。
開発側は lock を勝手に更新して素通りするが、本番側は不一致を検出して止まる
(これは正しい挙動で、**本番で入るバージョンを固定するための仕組み**。
緩めてはいけない)。

### 決定: `deploy.sh` でビルド前に弾く

「レビューで気をつける」ではなく、**機械が止める**形にする。
`localhost` の件で採った「手順を守るより間違えられない形」と同じ方針。

`infra/deploy.sh` のステップ `①.5` で、ビルドに入る前に両方を検査する:

| 検査 | 見るもの |
|---|---|
| (a) | `frontend/src` の `import` 先が `package.json` にあるか |
| (b) | `package.json` の依存が `pnpm-lock.yaml` にあるか |

CT には node も pnpm も入れていない(本番イメージは alpine + 静的バイナリ)ため、
**テキストの突き合わせだけ**で判定する。バージョンの不一致までは見ない。
数分かかるビルドを待たずに理由が分かれば目的は足りる。

> ✅ **2026-08-28 対応済み。** 本番構成を丸ごと起動するリハーサルで
> 1 を実際に踏み、`deploy.sh` にガードを追加した。
> `qrcode.react` は `package.json` / `pnpm-lock.yaml` へ同時に追加済み。

### 付随して直した `deploy.sh` のバグ

`.env.prod` の空欄チェックが、**キーが1行も無いときに何も言わずに終了していた。**

```bash
value="$(grep -E "^${key}=" .env.prod | head -1 | cut -d= -f2-)"
```

`set -euo pipefail` の `pipefail` により `grep` の終了コード 1 がパイプライン全体に伝播し、
「どの値が空か」を表示する前に `set -e` がスクリプトごと終了させていた。
**空欄を親切に教えるための仕組みが、空欄のときだけ動かない**状態。`|| true` で修正済み。

### `JOIN_URL` は値の有無だけでなく書式も見る

モニタが表示するQRコードは `JOIN_URL` の文字列をそのまま画像にしたもの。
**間違っていても backend は正常に起動する。**
気づくのは当日QRを読んだ参加者だけなので、`deploy.sh` で
`http(s)://` で始まり `/` で終わるかまで検査する。

なお実行時(API 経由)の値なので、**直したら backend の再起動だけで反映される。**
フロントの再ビルドは不要(`VITE_` 系との違いに注意)。
...
- [x] ~~本番構成の通しリハーサル~~ → 新規cloneから `deploy.sh` を完走。
      migrate / seed 5問 / SPAフォールバック(#50) / 管理者トークン認証 /
      進行制御4API / SSE疎通 / `dist` に `localhost` 無し、まで確認
- [x] ~~`qrcode.react` の依存漏れ~~ → リハーサルで検出。`package.json` と lock に追加
- [x] ~~`deploy.sh` のガード追加~~ → フロント依存(a)(b)・`JOIN_URL` の書式・
      `pipefail` で空欄チェックが黙って死ぬバグを修正
...
- [ ] 🔴 **SSE で `state` を push する**。現状は `hello` と15秒ハートビートのみで、
      フロントは `state` イベントを待っているため**画面が自動で切り替わらない**
      (リロードすれば正しい状態が出る)。当日のライブ進行にはこれが必須
- [ ] 🟡 `FinishedView` が `joinUrl` を使っている。終了画面に出すのはアンケートQR
      (API仕様書 §577)。`VITE_SURVEY_URL` の配線は済んでいるので、参照先の差し替えのみ
- [ ] 本番経路での200接続テスト(段取り④)
