...
- **4px の倍数でなくてよい。2の倍数まで許容する。** 端数の多くは光学調整であり、丸めると意図を壊すため
- **奇数 px は保留。** 扱いを検討中なので、**バインドしない**
- **`19px` → `20px` に変更する**（唯一の値変更。§3 参照）
...
| 対象 | 理由 |
| --- | --- |
| **奇数 px（`13px` `23px` `29px`）** | 扱いを検討中。バインドせず、件数と node ID を報告する |
...
## 3. Step 1: `19px` → `20px` の修正（値の変更。独立した作業）

**これは唯一「見た目が変わる」変更なので、バインド作業と混ぜない。別のコマンドまたは別モードとして実装する。**

- 対象: `Screens/Display` の選択肢グリッドの**交差軸 gap**（`counterAxisSpacing`）。再集計結果によれば 9件。代表 node: `525:3410` `525:3421` `525:3430`
- 主軸の gap は `16px` のまま。**変更しない**
- 実行前に、変更対象の node ID・現在値・変更後の値を**一覧で出力し、PM の確認を待つ**
- 実行後、PM に **before / after のスクリーンショット**を撮ってもらう旨を README に書く

⚠️ **`19px` を無条件に全部 `20px` にしてはいけない。** 交差軸 gap 以外の場所に `19px` があった場合（例: ステータスバーの `pb`）は対象外。**フレームとプロパティで限定すること。**
