...
func (h *Hub) Add() chan []byte {
	ch := make(chan []byte, 1) // ★ バッファ付き。最新の1件だけ持てばよい
	h.mu.Lock()
	h.clients[ch] = struct{}{}
	h.mu.Unlock()
	return ch
...
func (h *Hub) Broadcast(msg []byte) {
	...
	for ch := range h.clients {
		...
		case ch <- msg:
		default:
			// ★ 満杯 = 読み手が詰まっている。古い方を捨てて最新を入れる。
			//    新しい方を捨てると、reset 直後のように「次の配信が無い」場面で
			//    その端末が永久に古い画面のまま取り残される。
			select {
			case <-ch: // 最古を1件抜く(空なら default で素通り)
			default:
			}
			select {
			case ch <- msg:
...
**バッファ**と、**`select` の `default`**。

バッファ無しのチャネルに送ると、受け取る人が現れるまで**送る側がブロックする**。つまり電波の悪い1台が、`Broadcast` のループを止め、**残り199人への配信も止まる**。`default` を付けると「送れなければ何もしない」になり、これを防げる。

#### なぜバッファが 1 なのか

`state` は**差分ではなく毎回フルスナップショット**で、フロントは「受け取ったら画面全体を再描画する」だけ（→ API仕様書 §5「差分適用は不要」）。
つまり**過去の state を貯めても意味がない**。遅れている端末が古い画面を順に再生してから現在に追いつくだけになる。**最新の1件さえあれば正しい画面になる。**

#### なぜ「古い方」を捨てるのか

`select` + `default` を素直に書くと、満杯のときに捨てられるのは**今から入れようとしている最新の1件**になる（tail drop）。これは**間違い**。

```
reset {"to":"finished"} を配信 → その端末では捨てられる
→ その後クイズは終わっているので、管理者はもう何も操作しない
→ 次の配信が来ないので、その端末は「出題中」の画面のまま永久に取り残される
```

`waiting` / `finished` は**操作が止まる場所**なので、ここで落とすと回復手段が無くなる。だから**古い1件を抜いてから最新を入れる**。フルスナップショットなので途中を飛ばしても害はない。

> ⚠️ 内側の `<-ch` にも必ず `default:` を付けること。
> 空のチャネルからの受信は**ブロックする**。ここで止まると `h.mu` を握ったまま固まり、
> 配信も新規接続も切断処理も全部止まる（会場200台が同時に固まる）。
...
### ⚠️ 「DBから読む」から「配信する」までを排他する

Gin は**リクエストごとに goroutine を立てる**ので、この配信関数は**同時に複数走りうる**。
`loadSnapshot`（DB読み）と `BroadcastAll`（送信）の間で追い越されると、こうなる。

```
goroutine A: advance-text → DBが revealed=2 になる
goroutine A: DBから 2 を読む
             ★ ここで A が止まり、B に切り替わる
goroutine B: advance-text → DBが revealed=3 になる
goroutine B: 3 を読む → 配信      → 会場の画面が3セグメント目
goroutine A: 2 を配信            → 会場の画面が2セグメント目に巻き戻る
```

DB は正しく 3 のまま。**壊れるのは画面だけ**で、リロードするまで直らない。

対策は **`loadSnapshot` ごとロックの中に入れる**こと。
...
func broadcastState(db *gorm.DB, joinURL string, b *sse.Broadcaster) {
	...
	snap, err := loadSnapshot(db)   // ★ 読みをロックの中に入れるのが肝
	...
	b.BroadcastAll(a, m, p)
}
```

**読みが中に入っていることが重要。** 後から入った側は自分の番が来てから読み直すので、
古い値を持ったまま送ることが原理的に起きない。**順序を管理する仕組み（版番号など）は要らない。**

> フロント側でボタンを無効化していれば1タブからは追い越せないが、
> それは **UI の実装に依存した保証**であり、別タブ・別ボタン・再送では成立しない。
> ここでロックして、**バックエンド単体で順序を保証する**。
