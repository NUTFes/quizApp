...
| 5MBを超える | 413 | `FILE_TOO_LARGE` |

- **上限は nginx と Go の両方に設定する。** 本番は nginx(`client_max_body_size 5m`)が先に弾くが、**開発環境ではバックエンドを直接叩く(nginxを経由しない)ため Go 側にも上限が要る**。
