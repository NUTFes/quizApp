...
| 認証 | `Authorization: Bearer <ADMIN_TOKEN>` |
| 動画本体のサイズ上限 | **500MB** |
| リクエスト全体のサイズ上限 | **500MB + 64KB**(`512064k`) |
...
- 拡張子やブラウザが申告するContent-Typeは信頼せず、**ファイル先頭が `ftyp` ボックスで、major brandがMP4用(`isom` / `iso2` / `mp41` / `mp42` / `avc1` / `M4V ` / `M4A ` など)であること**を確認する。AVIF・HEIC・QuickTime MOVなど、拡張子を `.mp4` に変えただけの別形式は400で拒否する。
- バックエンドは「動画本体500MB」と「リクエスト全体500MB + 64KB」を分けて制限する。multipartの区切り・ヘッダ分を動画本体の上限に含めないため。
- nginxはこのAPI専用の `location /api/admin/videos` だけ `client_max_body_size 512064k`(**500MB + 64KB**)、`proxy_read_timeout 60s`、`proxy_send_timeout 60s` とする。`client_max_body_size` は動画本体ではなくmultipartの区切り・ヘッダも含むリクエスト全体を数えるため、500MBちょうどの動画を通す余裕として64KBを加える。Go側の `MaxRequestBytes` と同じ値に保つ。通常の `/api/` は画像投入に合わせた `5184k` のまま変更しない。
...
| `Authorization` ヘッダが無い/トークン不一致/`IMPORT_TOKEN` | 401 | `UNAUTHORIZED` |
| `multipart/form-data` でない / `file` フィールドが無い | 400 | `INVALID_REQUEST` |
| 先頭がMP4用major brandの `ftyp` ボックスでない | 400 | `INVALID_FILE_TYPE` |
...
- 敗者復活で流す動画はサーバーの `GET /videos/...` で配信する(認証なし)。画像と同じ `r.Static` の作りだが、**別ディレクトリ・別locationの独立した経路**(#121)。固定名は `/videos/revival.mp4` のみで、管理者PCの音声再生も同じファイルを使う(音声専用の配信経路やAPIは作らない)。
...
- 動画本体は500MBまで。管理者画面は送信前に確認ポップアップを出し、成功後に `/videos/revival.mp4` へのリンクを表示する。ページを開いた時点でも同URLへ `HEAD` を送り、200なら投入済みとして同じリンクを表示する(動画は1本だけなので一覧APIは作らない)。
- nginx の `location /videos/` も画像と同じく長期キャッシュせず(`max-age=300`)、`X-Content-Type-Options: nosniff` を付ける。
- **`.mp4` は Content-Type を明示登録する。** 実行環境(alpine)に `/etc/mime.types` が無く、Goの組み込みMIMEテーブルだけに任せられないため、`video/mp4` に固定する。登録しないとファイル内容のスニッフィング任せになり、nginx の `nosniff` と組み合わさってブラウザが再生を拒否する可能性がある(`backend/internal/platform/router.go` の `mime.AddExtensionType`)。
- `revival.mp4` は音声入りのまま保存し、無音版動画を別に作らない。通常はモニタの再生時にミュートし、管理者PCは同じ `revival.mp4` を `<audio>` として鳴らす。当日の切替はフロントのURLクエリだけで行い、API・SSEの契約は増やさない。音声だけの別ファイル(`revival.m4a`)は作らない。CTでの手動抽出作業(かつての `デプロイ手順.md` §5)は不要になった。
...
- 2026-09-19 第21版。**敗者復活動画の音声再生を、専用ファイルの手動抽出からrevival.mp4直接再生へ変更**した(#136追記)。①第20版で決めた「`ffmpeg`で抽出した `revival.m4a` をCTへ手動配置」は、本番投入直後にファイル未配置で音声が鳴らない事故が起きた上、そもそも「当日CTに触らない」(#158)方針と矛盾していたため廃止 ②管理者PCは投入済みの同じ `revival.mp4` を `<audio>` で再生する(`<audio>`タグは動画ファイルの音声トラックだけ再生できる) ③`.m4a` のMIME登録・配信経路・デプロイ手順書の抽出手順を削除。バックエンドAPI・SSEの契約は変わらない
- 2026-09-19 第20版。**敗者復活動画の音声分離再生を追加**した(#136)。①音入りの `revival.mp4` は変更せず、同じファイルから抽出した `/videos/revival.m4a` を既存の `/videos/` で配信する ②通常はモニタ動画をミュートし、「敗者復活へ」のAPI成功後に管理者PCで音声を再生する ③phase変更操作では音声を止める ④両画面へ `?revivalAudio=monitor` を付ければ、再デプロイせず音声出力をモニタPCへ移せる。バックエンドAPI・SSEの契約追加はない
...
- 2026-09-19 第18版。**音響連携をスコープ外から外した**(#133)。操作に紐づく効果音は音響スタッフが別途手動操作するのではなく、管理者画面の操作APIが成功した後に管理者PCで再生する。音を鳴らす処理はフロント内で完結し、バックエンドAPI・SSEの契約追加はない。担当範囲と失敗時の扱いは `画面・要件.md` §3「音響の担当範囲」に記載した
