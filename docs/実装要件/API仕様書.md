...
- 敗者復活で流す動画と抽出音声はサーバーの `GET /videos/...` で配信する(認証なし)。画像と同じ `r.Static` の作りだが、**別ディレクトリ・別locationの独立した経路**(#121)。動画は `/videos/revival.mp4`、音声は `/videos/revival.m4a` の固定名とし、音声用の新しい配信経路やAPIは作らない(#136)。
...
- **`.mp4` と `.m4a` は Content-Type を明示登録する。** 実行環境(alpine)に `/etc/mime.types` が無く、Goの組み込みMIMEテーブルだけに任せられないため、動画を `video/mp4`、抽出音声を `audio/mp4` に固定する。登録しないとファイル内容のスニッフィング任せになり、nginx の `nosniff` と組み合わさってブラウザが再生を拒否する可能性がある(`backend/internal/platform/router.go` の `mime.AddExtensionType`)。
- `revival.mp4` は音声入りのまま保存し、無音版動画を別に作らない。通常はモニタの再生時にミュートし、管理者PCから `revival.m4a` を鳴らす。当日の切替はフロントのURLクエリだけで行い、API・SSEの契約は増やさない。
- `revival.m4a` が無くても `revival.mp4` の配信・再生には影響させない。音声の抽出・配置・同期確認は [`デプロイ手順.md`](../ガイドライン/デプロイ手順.md) §5で行う。
